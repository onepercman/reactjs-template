...
interface SlipTabProps extends TabGroupProps<ElementType> {
  tabs?: string[]
  children?: React.ReactElement | React.ReactElement[]
}

export const SlipTab = React.forwardRef<HTMLDivElement, SlipTabProps>(
      ...
      <Tab.Group ref={ref} {...props}>
        {({ selectedIndex }) => (
          <React.Fragment>
            ...
            >
              {tabs?.map((tab, index) => (
                <Tab key={tab + index} className="relative z-10 h-10 flex-1 whitespace-nowrap rounded px-3">
                  {tab}
                </Tab>
              ))}
