...
export interface CollapseProps extends Omit<React.HTMLAttributes<HTMLDivElement>, "title" | "className"> {
...
export const Collapse = React.forwardRef<HTMLDivElement, CollapseProps>(function (
  { title, titleAfter, children, open, onToggle, defaultOpen, containerClassName, className, unmount, ...props },
  ref,
) {
  ...
  function _renderTitle(title: React.ReactNode | TitleRenderer) {
    ...
    return React.cloneElement(element as React.ReactElement, {
      className: cn("gap-base inline-flex h-8 w-full items-center justify-between", element.props.className),
      onClick() {
        if (height === 0 && childrenRef.current) {
          setIsOpen(true)
        } else {
  ...
  return (
    <div
      ref={composedRef}
      className={typeof className === "function" ? className({ isOpen: _isOpen }) : className}
      {...props}
    >
      {_renderTitle(title)}
      <div
        className={cn("w-full overflow-y-clip transition-all", containerClassName)}
        style={{
          height,
        }}
      >
