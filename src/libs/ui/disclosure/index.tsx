...
export const Disclosure = React.forwardRef<HTMLDivElement, DisclosureProps>(
  function ({ items, className, ...props }, ref) {
    const _className = cn(
      "bg-component flex flex-col gap-1 rounded p-1",
      className,
    )
            ...
            {({ open }) => (
              <React.Fragment>
                <HeadlessUI.Disclosure.Button
                  className={cn(
                    "bg-component relative z-10 rounded p-2 pr-8 text-left",
                  )}
                >
                  {item.label}
                  <HiChevronDown
                    className={cn(
                      "absolute right-2 top-3 transition-all",
                      open && "rotate-180",
                    )}
                  />
                </HeadlessUI.Disclosure.Button>
                <HeadlessUI.Transition appear show={open}>
                  ...
                  >
                    <HeadlessUI.Disclosure.Panel className="p-2">
                      {item.content}
