import { MenuSlotsClasses, MenuVariantProps, menu } from "@/libs/ui/theme"
...
export interface MenuProps extends Ark.MenuRootProps, MenuVariantProps, MenuSlotsClasses {
...
export const Menu = _constructor(function (
  { children, options = [], indent = 16, size, className, classNames, ...props },
  ref,
) {
  ...
  function _renderOption(option: MenuOption<true> | MenuOption<false>, offset = 0) {
    if ((option as MenuOption<true>).isSeparator === true) {
      ...
      return (
        <Ark.Menu.Separator className={classes.separator({ className, class: classNames?.separator })} {...separator} />
      )
    }
    option = option as MenuOption<false>
    if (option.items?.length)
      return (
        <Ark.Menu.ItemGroup className={classes.group({ class: classNames?.group })}>
          <Ark.Menu.ItemGroupLabel className={classes.groupLabel({ class: classNames?.groupLabel })}>
    ...
    if (option.children?.length)
      return (
        <Ark.Menu.Root>
          <Ark.Menu.TriggerItem className={classes.item({ class: classNames?.item })}>
            {option.label} <LuChevronRight />
          </Ark.Menu.TriggerItem>
          <Ark.Portal>
            <Ark.Menu.Positioner>
              <Ark.Menu.Content className={classes.list({ class: classNames?.list })}>
    ...
    return (
      <Ark.Menu.Item {...option} className={classes.item({ class: classNames?.item })}>
        {option.label}
      </Ark.Menu.Item>
    )
  }
  ...
  return (
    ...
    >
      ...
      <Ark.Portal>
        <Ark.Menu.Positioner>
          <Ark.Menu.Content ref={ref} className={classes.list({ className, class: classNames?.list })}>
