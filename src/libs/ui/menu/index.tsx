          ...
          {menu?.map((item, index) =>
            item.children?.length ? (
              <Menu
                ...
                key={index + id}
              >
                <span className="hover:bg-secondary/50 relative inline-flex w-full cursor-pointer items-center justify-between gap-2 rounded p-2">
              ...
              <HeadlessUI.Menu.Item key={index + id} disabled={item.disabled}>
                <span
                  className={cn(
                    "hover:bg-secondary/50 relative inline-flex cursor-pointer items-center gap-2 rounded p-2 pr-8 transition-colors",
